...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${transparent ? 'bg-transparent' : 'bg-white shadow-md'}`}>
      {demoVisible && (
        <div className="bg-amber-500 text-white text-xs py-2 px-4 text-center relative">
      ...
      <div className="container-xl">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <Link to="/" className="flex items-center gap-2.5 flex-shrink-0">
            <div className={`w-9 h-9 rounded-lg flex items-center justify-center font-black text-sm ${transparent ? 'bg-gold-500 text-white' : 'bg-navy-800 text-gold-400'}`}>
              IR
            </div>
            <span className={`font-bold text-xl tracking-tight ${transparent ? 'text-white' : 'text-navy-900'}`}>
              IRUR <span className={`font-normal text-sm ${transparent ? 'text-white/70' : 'text-gray-400'}`}>I Rent U Rent</span>
            </span>
          </Link>
              ...
              <Link key={l.to} to={l.to}
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                  location.pathname === l.to
                    ...
                    : transparent ? 'text-white/85 hover:text-white' : 'text-gray-600 hover:text-navy-800'
                }`}>
                {l.label}
              </Link>
            ))}
          ...
          {/* Mobile hamburger */}
          <button onClick={() => setMobileOpen(o => !o)} className={`lg:hidden p-2 rounded-lg ${transparent ? 'text-white' : 'text-navy-800'}`}>
            {mobileOpen ? (
