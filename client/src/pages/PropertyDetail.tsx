  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      ...
      <div className="pt-32 container-xl py-16">
        <div className="shimmer rounded-2xl aspect-[16/8] mb-8" />
      ...
      <div className="pt-24 lg:pt-28 pb-20">
        ...
        <div className="bg-white dark:bg-slate-950 border-b border-slate-200/80 dark:border-slate-800 py-3">
          <div className="container-xl flex items-center gap-2 text-xs font-medium text-slate-500 dark:text-slate-400">
            ...
            <span>/</span>
            <Link to="/properties" className="hover:text-brand-500 transition-colors">Properties</Link>
            <span>/</span>
              ...
              {photos.length > 1 && (
                <>
                  <button
                    ...
                    className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-md flex items-center justify-center shadow-md hover:scale-105 transition-all text-slate-800 dark:text-white"
                  >
                    ‹
                  </button>
                  <button
                    onClick={() => setActivePhoto(i => (i + 1) % photos.length)}
                    className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-md flex items-center justify-center shadow-md hover:scale-105 transition-all text-slate-800 dark:text-white"
                  >
                    ›
                  </button>
                  <div className="absolute bottom-4 right-4 bg-slate-900/80 backdrop-blur-md text-white text-xs font-semibold px-3 py-1 rounded-md">
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className="card p-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {[
                    ...
                    { label: 'Square Feet', val: property.sqft ? `${property.sqft.toLocaleString()} sqft` : 'N/A' },
                    { label: 'Property Type', val: property.property_type },
                    { label: 'Furnished', val: property.furnished ? 'Yes' : 'No' },
                    { label: 'Pet Friendly', val: property.pet_friendly ? 'Yes' : 'No' },
                  ].map((s) => (
                    <div key={s.label} className="bg-slate-50 dark:bg-slate-900 rounded-xl p-3.5 border border-slate-100 dark:border-slate-800">
              ...
              <div className="card overflow-hidden">
                <div className="p-6 pb-4">
                  <h2 className="font-display font-bold text-slate-900 dark:text-white text-base">Location</h2>
                </div>
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-28 space-y-6">
                ...
                <div className="card p-6">
                  ...
                  <div className="mb-6">
                    {property.status === 'available' && <span className="badge-available">● Available Now</span>}
                    {property.status === 'occupied' && <span className="badge-occupied">● Occupied</span>}
                    {property.status === 'maintenance' && <span className="badge-maintenance">● Maintenance</span>}
                  </div>

                  <a href="#inquiry-form" className="btn-primary w-full text-center py-3 mb-3">
                    Request a Tour
                  </a>
                  <Link to="/book-session" className="btn-secondary w-full text-center py-3 text-xs">
                ...
                <div className="card p-6 bg-slate-900 text-white">
                  <h3 className="font-display font-bold text-base mb-1">Questions About This Home?</h3>
                  <p className="text-slate-400 text-xs mb-4">Our property managers are standing by to assist you.</p>
                  <a href="tel:+17174336793" className="text-brand-400 font-bold text-sm block mb-1 hover:underline">(717) 433-6793</a>
                  <a href="mailto:info@irur.com" className="text-brand-400 text-xs hover:underline">info@irur.com</a>
                </div>
            ...
            <div className="card p-8">
              ...
              {submitted ? (
                <div className="text-center py-8">
                  <div className="w-12 h-12 rounded-full bg-emerald-100 dark:bg-emerald-950 text-emerald-600 dark:text-emerald-400 flex items-center justify-center text-xl font-bold mx-auto mb-3">✓</div>
